...
If you want to completly stop the black flicker on older devices then we have support for the ZOrderOnTop fix, warning though this will cause the view to overlap all views including your app chrome (i.e the actionbar). This behaviour is disabled by default but to opt in simple set useZOnTopFix to true.
